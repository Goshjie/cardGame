...
CardSprite* CardSprite::create(const CardModel* model)
{
...
bool CardSprite::init(const CardModel* model)
{
    ...
    int color_idx = (_model->suit == CardSuitType::Diamonds || _model->suit == CardSuitType::Hearts) ? 1 : 0;

    // 1. Big number in the center
    auto bigFaceSprite = Sprite::create(StringUtils::format("number/big_%s_%s.png", color_name[color_idx], face_name[(int)_model->face]));
    if (bigFaceSprite)
    {
    ...
    auto smallFaceSprite = Sprite::create(StringUtils::format("number/small_%s_%s.png", color_name[color_idx], face_name[(int)_model->face]));
    if (smallFaceSprite)
    {
    ...
    auto suitSprite = Sprite::create(StringUtils::format("suits/%s.png", suit_name[(int)_model->suit]));
    if (suitSprite)
    {
